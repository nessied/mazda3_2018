...
from openpilot.selfdrive.car.gm.values import DBC, AccState, CanBus, STEER_THRESHOLD, SC_CAR
...
class CarState(CarStateBase):
  ...
  def update(self, pt_cp, cam_cp, loopback_cp):
    ...
    cp = cam_cp if self.CP.carFingerprint in SC_CAR else pt_cp
    ...
    if self.CP.carFingerprint not in SC_CAR:
      ...
      self.loopback_lka_steering_cmd_updated = len(loopback_cp.vl_all["ASCMALKASteeringCmd"]["RollingCounter"]) > 0
      if self.loopback_lka_steering_cmd_updated:
        self.loopback_lka_steering_cmd_ts_nanos = loopback_cp.ts_nanos["ASCMALKASteeringCmd"]["RollingCounter"]
  ...
  def get_can_parser(CP):
    ...
    if CP.networkLocation == NetworkLocation.fwdCamera and CP.carFingerprint not in SC_CAR:
      messages.append(("ASCMLKASteeringCmd", 0))

    if CP.transmissionType == TransmissionType.direct:
      if CP.carFingerprint in SC_CAR:
        messages.append(("EBCMRegenPaddle", 40))
      else:
  ...
  def get_loopback_can_parser(CP):
    messages = []
    if CP.carFingerprint in SC_CAR:
      messages.append(("ASCMALKASteeringCmd", 0))
    else:
      messages.append(("ASCMLKASteeringCmd", 0))
