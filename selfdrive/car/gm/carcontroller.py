...
from openpilot.selfdrive.car.gm.values import DBC, CanBus, CarControllerParams, CruiseButtons, GMFlags, CAMERA_ACC_CAR, CC_ONLY_CAR, SDGM_CAR, EV_CAR, AccState
    ...
    if self.CP.openpilotLongitudinalControl and not frogpilot_toggles.CSLC:
      ...
      if (self.CP.carFingerprint in SDGM_CAR or self.CP.carFingerprint in CAMERA_ACC_CAR) and frogpilot_toggles.CSLC:
        ...
        if CC.enabled and self.frame % 3 == 0 and CS.cruise_buttons == CruiseButtons.UNPRESS and not CS.out.gasPressed and not CS.distance_button:
          ...
          can_sends.extend(gmcan.create_gm_acc_spam_command(self.packer_pt, self, CS, slcSet, bus, CS.out.vEgo, frogpilot_toggles, accel, experimentalMode, self.CP.carFingerprint in SDGM_CAR))
        ...
        if self.frame % 4 == 0:
          # Send dashboard UI commands (ACC status)
          send_fcw = hud_alert == VisualAlert.fcw
          can_sends.append(gmcan.create_acc_dashboard_command(self.packer_pt, CanBus.POWERTRAIN, CC.enabled, hud_v_cruise * CV.MS_TO_KPH, hud_control, send_fcw))
