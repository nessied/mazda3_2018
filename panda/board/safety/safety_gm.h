...
const CanMsg GM_CAM_TX_MSGS[] = {{0x180, 0, 4}, {0x200, 0, 6}, {0x1E1, 0, 7}, {0x370, 0, 6}, // pt bus
                                 {0x1E1, 2, 7}, {0x184, 2, 8}};  // camera bus

const CanMsg GM_CAM_LONG_TX_MSGS[] = {{0x180, 0, 4}, {0x315, 0, 5}, {0x2CB, 0, 8}, {0x370, 0, 6}, {0x200, 0, 6},  // pt bus
                                      {0x1E1, 2, 7}, {0x184, 2, 8}};  // camera bus

const CanMsg GM_SDGM_TX_MSGS[] = {{0x180, 0, 4}, {0x1E1, 0, 7}, {0x370, 0, 6}, // pt bus
...
static int gm_fwd_hook(int bus_num, int addr) {
  ...
  if ((gm_hw == GM_CAM) || (gm_hw == GM_SDGM)) {
    ...
    if (bus_num == 2) {
      ...
      bool cslc_msg = (addr == 0x370);
      bool block_msg = is_lkas_msg || (is_acc_msg && gm_cam_long) || (cslc_msg && gm_cslc);
      if (!block_msg) {
        bus_fwd = 0;
      }
    }
